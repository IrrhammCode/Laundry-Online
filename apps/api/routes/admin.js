...
router.patch('/orders/:id/status', [
  ...
  try {
    ...
    // Note: DICUCI -> MENUNGGU_KONFIRMASI_DELIVERY should use confirm-delivery endpoint
    // Note: MENUNGGU_KONFIRMASI_DELIVERY -> MENUNGGU_AMBIL_SENDIRI/DIKIRIM should be done by user
    const validTransitions = {
      'DIPESAN': ['PESANAN_DIJEMPUT', 'DICUCI'], // PESANAN_DIJEMPUT untuk PICKUP (setelah approve), DICUCI untuk SELF
      'PESANAN_DIJEMPUT': ['DIAMBIL'], // Setelah dijemput, status menjadi DIAMBIL
      'DIAMBIL': ['DICUCI'], // Setelah diambil, masuk ke proses cuci
      ...
      'MENUNGGU_AMBIL_SENDIRI': ['SELESAI'], // User ambil sendiri
      'DIKIRIM': ['SELESAI'], // Setelah dikirim, selesai
      'SELESAI': []
    };
...
router.get('/dashboard/stats', async (req, res) => {
  try {
    ...
    const completedOrders30d = completedOrders?.length || 0;

    // Get pending orders
    const { count: pendingOrders, error: pendingError } = await db
      .from('orders')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'DIPESAN');
...
router.get('/notifications', async (req, res) => {
  try {
    ...
    // Get notifications for admin (all notifications, not just for specific user)
...
router.get('/reviews', async (req, res) => {
  try {
    ...
    const { data: reviews, error: reviewsError } = await db
      .from('reviews')
      .select('id, order_id, user_id, service_id, rating, comment, created_at')
      .order('created_at', { ascending: false })
      .range(offset, offset + parseInt(limit) - 1);
    ...
    const reviewsWithDetails = await Promise.all(
      (reviews || []).map(async (review) => {
        const [userData, orderData, serviceData] = await Promise.all([
          db.from('users').select('name, email').eq('id', review.user_id).single(),
          db.from('orders').select('id, price_total, status').eq('id', review.order_id).single(),
          review.service_id ? db.from('services').select('name').eq('id', review.service_id).single() : Promise.resolve({ data: null })
        ]);

        return {
          id: review.id,
          order_id: review.order_id,
          user_name: userData.data?.name || 'Unknown',
          user_email: userData.data?.email || '',
          service_name: serviceData.data?.name || null,
          rating: review.rating,
          comment: review.comment,
          created_at: review.created_at,
          order_total: orderData.data?.price_total || 0,
