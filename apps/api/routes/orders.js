...
router.post('/', [
  ...
  body('notes').optional({ nullable: true, checkFalsy: true }).trim().isLength({ max: 500 }).withMessage('Notes too long'),
  body('notification_email').optional({ nullable: true, checkFalsy: true }).isEmail().withMessage('Invalid email address')
], async (req, res) => {
  try {
    ...
    const orderItems = [];
      ...
      // User details already fetched above, no need to query again
...
// Get Recommended Packages (must be before /:id route)
...
// User: Choose Delivery Method (setelah admin confirm delivery)
router.patch('/:id/choose-delivery', [
  body('delivery_method').isIn(['SELF_PICKUP', 'DELIVERY']).withMessage('delivery_method must be SELF_PICKUP or DELIVERY')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    if (delivery_method === 'SELF_PICKUP') {
      ...
      const totalWithDelivery = parseFloat(orderData.price_total) + deliveryFee;

      // Update order - set delivery_required dan status menunggu payment
      const { error: updateError } = await db
        .from('orders')
        .update({ 
      ...
      await db
        ...
        .insert({
          ...
          payload_json: { 
            ...
            isi_pesan: `Order #${orderId} memerlukan pembayaran delivery fee sebesar Rp ${deliveryFee.toLocaleString('id-ID')}.`
          },
          channel: 'EMAIL'
        });
      ...
      res.json({
        ok: true,
        message: 'Delivery method selected. Please complete payment.',
        data: {
          status: 'MENUNGGU_PEMBAYARAN_DELIVERY',
          delivery_method: 'DELIVERY',
          delivery_fee: deliveryFee,
    ...
    console.error('Choose delivery error:', error);
    res.status(500).json({
      ok: false,
...
// User: Pay QRIS (Mock Payment)
router.post('/:id/pay-qris', async (req, res) => {
  try {
    const orderId = req.params.id;
    const userId = req.user.id;
    ...
    // Only allow for orders waiting for payment
    if (orderData.status !== 'MENUNGGU_PEMBAYARAN_DELIVERY') {
      return res.status(400).json({
        ok: false,
    ...
    // Mock payment - langsung set paid
    ...
    await db
      ...
      .insert({
        ...
        payload_json: { 
          isi_pesan: `Pembayaran delivery fee untuk order #${orderId} berhasil. Pesanan sedang dikirim.`
        },
        channel: 'EMAIL'
      });
